...
import { HelpNode, HelpPage, InlineNode, GeneratorConfig, HighlightGrammarModule, ValidationWarning } from './types';
...
export class Generator {
  ...
  private allPageIds: Set<string> = new Set();
  private highlightGrammar: HighlightGrammarModule | null = null;
  ...
  constructor(config: GeneratorConfig) {
    ...
    this.preprocessor = new Preprocessor(config.inputDir);
   ...
   */
  async generateAll(): Promise<ValidationWarning[]> {
  ...
  async generateFile(filename: string): Promise<void> {
    ...
    const page: HelpPage = { meta: result.meta, nodes };
   ...
   */
  private generatePageCode(page: HelpPage, dynamicKeys: string[], linkedPages: string[], sourceFile: string): string {
   ...
   */
  private generateNodeCode(node: HelpNode, prefix: string, parentVar: string): string[] {
    const lines: string[] = [];
      ...
      case 'codeBlock': {
        ...
        if (isDisplayMode) {
          ...
          if (grammar) {
            ...
            const tokens = Prism.tokenize(node.content, grammar as Prism.Grammar);
   ...
   */
  private async loadHighlightGrammar(): Promise<HighlightGrammarModule | null> {
    if (!this.config.grammarPath) return null;
    const url = pathToFileURL(path.resolve(this.config.grammarPath)).href;
    ...
    const module = (imported.default ?? imported) as Partial<HighlightGrammarModule>;
    ...
    return module as HighlightGrammarModule;
  }
   ...
   */
  private generateStaticTokenCode(tokens: Array<string | Prism.Token>, parentVar: string): string[] {
    const lines: string[] = [];
    for (const token of tokens) {
      if (typeof token === 'string') {
        if (token) lines.push(`${parentVar}.appendText(${JSON.stringify(token)});`);
        continue;
      }
  ...
  private generatePrismTokenRendererCode(): string[] {
    return [
   ...
   */
  private generateInlineContainerCode(children: InlineNode[], prefix: string, parentVar: string, tag: string, cls: string, context: string): string[] {
    const lines: string[] = [];
   ...
   */
  private generateInlineCode(node: InlineNode, prefix: string, parentVar: string): string[] {
    const lines: string[] = [];
   ...
   */
  private generateImageCode(node: { type: 'inlineImage'; src: string; alt: string; title?: string }, prefix: string, parentVar: string): string[] {
    const lines: string[] = [];
    const attrs: string[] = [
   ...
   */
  private generateListCode(node: { type: 'list'; ordered: boolean; items: Array<{ type: 'listItem'; children: InlineNode[]; nestedList?: any }> }, prefix: string, parentVar: string): string[] {
    const lines: string[] = [];
   ...
   */
  private generateBlockquoteCode(node: { type: 'blockquote'; children: HelpNode[] }, prefix: string, parentVar: string): string[] {
    const lines: string[] = [];
   ...
   */
  private generateCalloutCode(node: { type: 'callout'; calloutType: string; title?: string; children: HelpNode[] }, prefix: string, parentVar: string): string[] {
    const lines: string[] = [];
   ...
   */
  private generateTableCode(node: { type: 'table'; headers: string[]; rows: string[][] }, prefix: string, parentVar: string): string[] {
    const lines: string[] = [];
   ...
   */
  private generateLinkCode(node: { type: 'link'; pageId: string; text?: string }, prefix: string, parentVar: string): string[] {
    const lines: string[] = [];
...
`;
   ...
   */
  private async generateIndex(mdFiles: string[]): Promise<void> {
    const lines: string[] = [];
    const pages: Array<{ varName: string; importPath: string }> = [];
   ...
   */
  private getSemanticVar(context: string): string {
   ...
   */
  private hasInlineFormatting(text: string): boolean {
