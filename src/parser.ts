import type { HelpNode, HeadingNode, ParagraphNode, ListNode, ListItemNode, TableNode, CodeBlockNode, DynamicNode, LinkNode, HtmlNode, InlineNode, TextNode, BoldNode, ItalicNode, StrikethroughNode, CodeNode, InlineLinkNode, InlineImageNode, InlineDynamicNode, BlockquoteNode, CalloutNode, HorizontalRuleNode } from './types';
...
export class Parser {
  ...
  parse(content: string): HelpNode[] {
    ...
    while (i < lines.length) {
      ...
      if (headingMatch) {
        ...
        const heading: HeadingNode = { type: 'heading', level, text, children: this.parseInline(text) };
        nodes.push(heading);
        i++;
        continue;
      }
   ...
   */
  private parseList(lines: string[], startIndex: number, ordered: boolean, indent: number = 0): { node: ListNode; nextIndex: number } {
    ...
    while (i < lines.length) {
      ...
      if (match) {
        const item: ListItemNode = { type: 'listItem', children: this.parseInline(match[1]) };
   ...
   */
  private parseBlockquote(lines: string[], startIndex: number): { node: BlockquoteNode; nextIndex: number } {
   ...
   */
  private parseCallout(lines: string[], startIndex: number, calloutType: string, title?: string): { node: CalloutNode; nextIndex: number } {
    const quotedLines: string[] = [];
   ...
   * Parse a GFM table
   */
  private parseTable(lines: string[], startIndex: number): { node: TableNode; nextIndex: number } | null {
   ...
   */
  private isParagraphBreak(line: string, followingLine?: string): boolean {
